...
public class AG_Reco_Voz extends PApplet {
...
public void setup() {
  ...
  for(int i=0; i<pop; i++){
    for(int j=0; j<l; j++){
      cromosomas[i][j]=random(-0.5f,0.5f);
    }
  }
...
public void draw() {
    ...
    if(millis()-tiempo>2500 && gen<=max_gen){
      ...
      for(int i=0; i<pop; i++){
        ...
        for(int t=0; t<l; t++){
            ...
            float penalizacion=pow(abs(errores[t]),1/2.0f);          //Penalizacion Fuerte
      ...
      //  print(sel+" -- ");
        ...
        for(int t=0; t<l; t++){
            ...
            if(mutar<abs(error[sel][t])){
              float aleatorio = random(min(0,1.5f*error[sel][t]),max(0,1.5f*error[sel][t]));
              if(t>=90 && t<95){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "AG_Reco_Voz" };
    if (passedArgs != null) {
